  ...
  describe("Administrative functions", () => {
    ...
    describe("Add supported tokens", () => {
      it("Should add supported tokens successfully", async () => {
        ({ proxyBridge, testToken, defaultAdminSigner } = await loadFixture(
          deployContracts
        ));
      ...
      it("Failed to add because of unauthorization", async () => {
        ({ proxyBridge, testToken, arbitrarySigner } = await loadFixture(
          deployContracts
        ));
        expect(await proxyBridge.supportedTokens(testToken.address)).to.equal(
          false
    ...
    describe("Remove supported token", () => {
      it("Should remove supported token successfully", async () => {
        ...
        await expect(
          proxyBridge
            .connect(defaultAdminSigner)
            .removeSupportedToken(testToken.address)
        )
      ...
      it("Can't remove supported token because of not being authorized", async () => {
        ...
        await expect(
          ...
          "AccessControl: account " +
            arbitrarySigner.address.toLowerCase() +
            " is missing role " +
            "0x" +
            "0".repeat(64)
        );
      ...
      it("Should not remove because the token is not supported yet", async () => {
        ({ defaultAdminSigner, testToken } = await loadFixture(
          deployContracts
        ));
        await expect(
          proxyBridge
    ...
    describe("Change Tx Fee", () => {
      ...
      it("Should not be able to change tx fee if not authorized", async () => {
        ({ proxyBridge, testToken, defaultAdminSigner, arbitrarySigner } =
          await loadFixture(deployContracts));
        await expect(proxyBridge.connect(arbitrarySigner).changeTxFee(1000)).to
          .be.reverted;
      ...
      it("Should not be able to make tx Fee higher than maximum", async () => {
        ...
        await expect(proxyBridge.connect(defaultAdminSigner).changeTxFee(10001))
          .to.be.reverted;
      });
    });
    ...
    describe("Change cold wallet", () => {
      ...
      it("Should not be able to change cold wallet because of being unauthorized", async () => {
        ...
        expect(await proxyBridge.coldWallet()).to.equal(
          coldWalletSigner.address
        );
      });

      it("Should not be able to change cold wallet if new address is zero address", async () => {
        ({ proxyBridge, defaultAdminSigner, arbitrarySigner } =
          await loadFixture(deployContracts));
        await expect(
          proxyBridge
            .connect(defaultAdminSigner)
            .changeColdWallet(ethers.constants.AddressZero)
    ...
    describe("Change community wallet", () => {
      ...
      it("Should not be able to change community wallet if new address is zero address", async () => {
        ...
        await expect(
          proxyBridge
            .connect(defaultAdminSigner)
            .changeCommunityWallet(ethers.constants.AddressZero)
        ).to.be.revertedWithCustomError(
          proxyBridge,
          "INVALID_COMMUNITY_WALLET"
        );
      });
    });
  });
  ...
  describe("Handling bridging request", () => {
    describe("Test bridgeToDeFiChain function for ETH token", () => {
      ...
      it("Revert when the _amount input is non-zero", async () => {
        const { proxyBridge, defaultAdminSigner } = await loadFixture(
          deployContracts
        );

        await proxyBridge
          .connect(defaultAdminSigner)
          .addSupportedToken(ethers.constants.AddressZero);

        expect(await ethers.provider.getBalance(proxyBridge.address)).to.equal(
          0
        );
        await expect(
          proxyBridge.bridgeToDeFiChain(
          ...
          proxyBridge,
          "AMOUNT_PARAMETER_NOT_ZERO_WHEN_BRIDGING_ETH"
        );
      });
      ...
      it("Revert when the ETH amount requested is zero", async () => {
        ...
        await expect(
          ...
          proxyBridge,
          "REQUESTED_BRIDGE_AMOUNT_IS_ZERO"
        );
      });
      ...
      it("Should be able to bridge when all requirements are satisfied", async () => {
        ...
        expect(
          await ethers.provider.getBalance(coldWalletSigner.address)
        ).to.equal(toWei("10000"));
        expect(
          await ethers.provider.getBalance(communityWalletSigner.address)
        ).to.equal(toWei("10000"));
      ...
      it("Change the txFee, should calculate correctly the amount being bridged", async () => {
        ...
        expect(
          await ethers.provider.getBalance(coldWalletSigner.address)
        ).to.equal(toWei("10000.9"));
        expect(
          await ethers.provider.getBalance(communityWalletSigner.address)
        ).to.equal(toWei("10000.1"));
      });
    });
    ...
    describe("Test bridgeToDeFiChain function for ERC20 token", () => {
      ...
      it("Revert when the msg.value input is non-zero", async () => {
        ...
        await expect(
          ...
          proxyBridge,
          "MSG_VALUE_NOT_ZERO_WHEN_BRIDGING_ERC20"
        );
      });
      ...
      it("Should be able to bridge when all the requirements are satisfied", async () => {
        ...
        expect(
          await testToken.balanceOf(communityWalletSigner.address)
        ).to.equal(0);
