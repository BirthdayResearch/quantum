...
import { BridgeQueueProxy, BridgeQueue__factory } from "../generated";
...
export async function deployBridgeProxy({
  ...
  const bridgeProxyContract = await ethers.getContractFactory(
    "BridgeQueueProxy"
  );
  const encodedData = BridgeQueue__factory.createInterface().encodeFunctionData(
    "initialize",
    [timelockContractAddress, coldWalletAddress, fee, communityWalletAddress]
  );
  const bridgeQueueProxy = await bridgeProxyContract.deploy(
    bridgeQueueAddress,
    encodedData
  );
