...
export class DeFiChainContainer {
    ...
    public localMnemonic: string = 'avoid between cupboard there nerve sugar quote foot broom intact seminar culture much anger hold rival moral silly volcano fog service decline tortoise combine';
    constructor(protected defid?: StartedNativeChainContainer,
    ...
    async start(): Promise<string> {
        ...
        this.playground = await new PlaygroundApiContainer().withNetwork(network).withNativeChain(this.defid, network).start();
    ...
    async generateBlock(): Promise<void> {
        await this.playgroundClient?.rpc.call(
            "generatetoaddress",
            [3, "mswsMVsyGMj1FzDMbbxw2QW3KvQAv2FKiy"],
            "number"
        );
    }
}